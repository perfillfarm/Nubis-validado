...
export async function createParadiseTransaction(
  ...
  try {
    ...
    const transaction: Transaction = {
      ...
      cpf: data.cpf,
      external_transaction_id: (paradiseTransaction.transaction_id || paradiseTransaction.id || transactionId).toString(),
      provider: 'paradise',
...
export async function getParadiseTransactionStatus(
  ...
  try {
    ...
    const response = await fetch(
      `${config.apiUrl}/api/v1/query.php?action=get_transaction&id=${transactionId}`,
      {
        method: 'GET',
        headers: {
          'X-API-Key': config.secretKey,
